...
export default function AISection() {
  return (
    <section id="ai-automation" className="py-20 sm:py-28 bg-surface-alt/30 border-t border-border/40">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {AI_AUTOMATION_PRACTICES.map((item, i) => (
            <motion.div
              key={item.title}
              className="p-6 rounded-xl border border-border bg-surface flex flex-col justify-between hover:border-accent/40 transition-colors group"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="text-2xl p-2 rounded-lg bg-surface-alt border border-border">
                    {item.icon}
                  </span>
              ...
              <div className="pt-4 border-t border-border/60 flex flex-wrap gap-1.5">
                {item.tools.map((tool) => (
                  <span
                    key={tool}
                    className="px-2 py-0.5 text-xs font-mono bg-surface-alt text-muted rounded border border-border"
                  >
                    {tool}
                  </span>
                ))}
